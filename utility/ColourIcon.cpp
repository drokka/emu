...
emu::utility::ColourIcon::ColourIcon(int xSz, int ySz, double *bgRGBA, double *minRGBA, double *maxRGBA,
                                     PointList *pointList,
                                     ColourFn colourFn, RgbaList2D &clrArray) : xSz(xSz), ySz(ySz), bgRGBA(bgRGBA),
                                                                                minRGBA(minRGBA), maxRGBA(maxRGBA),
                                                                                pointList(pointList),
                                                                                colourFn(colourFn),
                                                                                colourArray(clrArray) {
}
...
emu::utility::ColourIcon::colourIn(int sz, bool argb, unsigned char **rgbaByteArray, int nperiod,
    ...
    cout << "save max and range: " << fd->getMinX() << " " << fd->getMaxX() << " " << fd->getMinY() << " "
         << fd->getMaxY()
         << " "
...
// Set background colour for all pixels
  ...
  //       cout<< "found!! " <<hits <<endl;
        ...
        // Set the colour for this number of hits
  ...
  //      if (nperiod == 1) {
     ...
     /*   }else  if(nperiod > 1) {  //AND HEX!!!!!!!!!!
            x = x/nperiod; y = y/nperiod;  //scale down by period.
            for (int i = -nperiod / 2 - 1; i < nperiod / 2 + 2; i++) {
                for (int j = 0; j < nperiod + 1; j++) {
                    int xx = (x + i + j * 0.5) * sz /
                             nperiod;  //   k11, k12 (1,0)    k21,k22 (0.5, sq3/2) // (NumXPixel  * x / Info.degreeSym )

                    int yy = sz - (sz / nperiod) * (y + j * sq3 * 0.5);
                    if (xx < 0 || xx >= sz || yy < 0 || yy >= ySz) {
                     //   cout << "outside of boundary x = " << x << " , y = " << y << endl;
                        continue; // ignore values outside of range - what happened to scaling?
                    }
                    for (int i = 0; i < 4; i++) {
                        (*rgbaByteArray)[4 * (yy * iconSize + xx) + i] = (unsigned char) (rgba[i] * 255.0);
                    }
                }
            }
        }*/
